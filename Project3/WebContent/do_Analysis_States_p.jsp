...
<body>
	...
	<%
		...
		try {
			...
			{
				SQL_1 = "select s.name, pc.total from states as s, pc_StateCatAmt as pc where s.name = pc.state and pc.cid = "+ category +" order by pc.total desc limit 20";
				SQL_2 = "select pspa.pid, p.name, pspa.total from pc_StateProdAmt as pspa, products as p where p.id = pspa.pid and p.cid = "+ category +" order by pspa.total desc limit 10";
			...
			{
				SQL_1 = "select states.name, psa.total from states, pc_StateAmt as psa "+
			            "WHERE states.name = psa.state AND states.name = '"+state+
			            "' order by psa.total desc limit 20";
				SQL_2 = "select pspa.pid as id, products.name, pspa.total from pc_StateProdAmt as pspa, products, states "+
			            "where states.name = pspa.state and states.name = '"+state+
			            "' and products.id = pspa.pid order by pspa.total desc limit 10";
				SQL_4 = "DROP TABLE IF EXISTS temp1; CREATE TABLE temp1 (u_rank SERIAL PRIMARY KEY, sid INT); "+
						"INSERT INTO temp1(sid) select states.id from states, pc_StateAmt as psa "+
				        "WHERE states.name = psa.state AND states.name = 'Kansas' order by psa.total desc limit 20; "+
						"DROP TABLE IF EXISTS temp2; CREATE TABLE temp2 (p_Rank SERIAL PRIMARY KEY, pid INT); "+
				        "INSERT INTO temp2(pid) select pspa.pid from pc_StateProdAmt as pspa, "+
						"states where states.name = pspa.state and states.name = 'Kansas' order by pspa.total desc limit 10; "+
				        "DROP TABLE IF EXISTS temp3; CREATE TABLE temp3 (t_rank SERIAL PRIMARY KEY, sid INT, pid INT); "+
						"INSERT INTO temp3(sid, pid) select t1.sid, t2.pid from temp1 as t1, temp2 as t2;";
			...
			System.out.println("STATR SQL_@ QUERY");
	...
	<table align="center" width="100%" border="1">
		<tr align="center">
			<td><strong><font size="+2" color="#FF00FF">STATES</font></strong></td>
